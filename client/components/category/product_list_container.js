...
const query = gql`
  query getCategoryQuery(
    ...
    getCategory(slug: $slug) {
      ...
      productFeed(cursor: $cursor, colors: $colors, material: $material)
        @connection(key: "productFeed", filter: ["colors", "materials"]) {
        cursor
        hasMore
        products {
          name
          ...
          <div className="col-sm-8">
              ...
              {data.getCategory.productFeed.products.map((p, index) => {
                return (
                  <div key={index} className="col-sm-3">
                    <ProductThumb
                      color={
                        filters.colors.length > 0 ? filters.colors[0] : null
                      }
                      product={p}
                    />
                  </div>
            ...
            <div className="row">
              {data.getCategory.productFeed.hasMore ? (
                <button onClick={e => loadMoreProducts(filters)}>
                  Load more
                </button>
              ) : (
                ''
      ...
      loadMoreProducts: filters => {
        return props.data.fetchMore({
          ...
          updateQuery(previousResult, { fetchMoreResult }) {
            ...
            const newGetCategoryData = Object.assign(
              {},
              previousResult.getCategory,
              {
                productFeed: Object.assign(
                  {},
                  previousResult.getCategory.productFeed,
                  {
